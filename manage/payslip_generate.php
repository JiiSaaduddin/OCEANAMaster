<?php
	...
	FROM attendance LEFT JOIN employees ON employees.id=attendance.employee_id 
	...
	while($row = $query->fetch_assoc()){
		...
		$TAX = "0.00";
		$SSS = $row['gsss'];
		$PAGIBIG = $row['gpagibig'];
		$PHILHEALTH = $row['gphilhealth'];
		$rateD = $row['rate'] * 8 * 26; 
		$rateDhalf = $rateD / 2;
		...
		$gross = ($grossD + $grossF);
		$total_deduction = ($deduction + $cashadvance + $SSS); 
		$net = ($grossD + $grossF) - ($total_deduction);
;

		$contents .= '
			<h2 align="center" style="font-weight:300">OCEANA HR Solutions<br><small><b>ANA`S BREEDERS FARMS, INC.</b> - PaySlip</small><br><small>Payroo Period ('.$from_title." - ".$to_title.')</small></h2>
			<table cellspacing="0" cellpadding="3">
    	       	<tr>
				<td width="25%" align="right">Employee Name: </td>
				<td width="25%"><b>'.$row['firstname']." ".$row['lastname'].'</b></td>
			<td width="25%" align="right">Basic Rate: </td>
				<td width="25%" align="right">P'.($row['rate']=='0.00'?($row['rateflat'] / 2):number_format($rateDhalf,2)).'</td>
    	    	</tr>
    	    	<tr>
			...
			<td width="25%" align="right">'.number_format($row['total_hr'], 2).'</td>
    	    	...
    	    	<tr>
			<td width="25%" align="right">Regular Rate: </td>
			<td width="25%" align="left">P'.($row['rate']=='0.00'?$row['rateflat']:$rateD.'<br>(P'.$row['rate'].'&nbsp;daily)').'</td>
   
			<td width="25%" align="right"><b>GROSS PAY: </b></td>
			<td width="25%" align="right"><b>P'.number_format(($gross), 2).'</b></td>
		...
		<tr>
			...
			<td></td>
			<td></td>
			<td width="25%" align="right">Withholding Tax:</td>
			<td width="25%" align="right">'.$TAX.'</td>
    	    	</tr>
    	    	<tr>
			<td></td>
			<td></td>
			<td width="25%" align="right">Adjustment Allowance: </td>
			...
			<td width="25%" align="right"><b>'.number_format($total_deduction, 2).'</b></td>
    	    	...
    	    	<tr>
    	    		...
    	    		<td></td>
				 	...
				 	<td width="25%" align="right"><b>P'.number_format($net, 2).'</b></td>
    	    	</tr>
    	    </table>
    	    <br><hr>
		';
